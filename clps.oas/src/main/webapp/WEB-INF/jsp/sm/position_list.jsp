				...
				<div class="panel panel-default">
				    <div class="panel-heading">
						<div class="row">
							...
							<form class="bs-example bs-example-form col-md-5" role="form" style="margin: 20px 0 10px 0;" action="positionSelectByName?name=" id="form1" method="post">
								<div class="input-group">
									<input type="text" class="form-control" placeholder="请输入职位名称" name="findByName">
									<span class="input-group-addon btn" id="sub">搜索</span>
								</div>
							</form>
							<button class="btn btn-default col-md-2" style="margin-top: 20px" onClick="location.href='positionAdd?positionId=${requestScope.position.positionId}'">
								添加职位信息
					...
					<table class="table table-bordered">
						...
						<%i=i+1; %>
							<tr>
								...
								<td><nobr><fmt:formatDate value="${position.positionCreatedDatetime}" type="both"/></nobr></td>
								<td>${position.department.account.accountName}</td>
								<td>
										<button class="btn btn-default btn-xs btn-info" onClick="location.href='positionUpdate?positionId=${position.positionId}'">修改</button>
										<button class="btn btn-default btn-xs btn-danger btn-primary" onClick="location.href='positionDel?positionId=${position.positionId}'">删除</button>
										<!--弹出框-->
									</td>
							</tr>
						</c:forEach>
						...
						<c:if test="${pagingVO != null}">
							<nav style="text-align: center">
								<ul class="pagination">
									...
									<li><a
										href="positionList?page=${pagingVO.totalCountPage}">最后一页&raquo;</a></li>
									<c:if
