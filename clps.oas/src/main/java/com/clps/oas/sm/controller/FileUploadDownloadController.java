...
public class FileUploadDownloadController {
	...
	// 定位到上传的文件界面 /clps.mms/util/fileUpload
	@RequestMapping(value = "/fileUpload", method = RequestMethod.GET)
	public String showUploadPage() {
		return "util/file_upload";
	}
	...
	public String doUploadFile(@RequestParam("file") MultipartFile file, HttpServletRequest request) {
		...
		if (!file.isEmpty()) {
			...
			logger.info("导入Excel不存在！！！");
			return "redirect:/file/fileUpload";
		}
		logger.info("导入Excel到:" + path + newFileName);

		return "util/file_upload_success";
	}

	// 定位到文件列表显示 /clps.mms/util/fileList
	...
	public void downFile(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 如果文件不存在
			if (!file.exists()) {
				request.setAttribute("message", "您要导出的Excel已被删除！！");
				logger.info("您要导出的Excel已被删除！！");
			}
			// 处理文件名
