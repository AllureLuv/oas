/**    
* @Title: IDepartmentService.java  
* @Package com.clps.oas.service  

* @author ft   
* @date 2018年5月7日 下午2:16:27  
* @version V1.0    
*/
package com.clps.oas.sm.service;

import java.util.List;

import com.clps.oas.sm.pojo.Department;

/**
 * 
 * @desc: clps.oas
 * @author: noah.zhou
 * @createTime: 2019年1月28日 上午10:47:36
 * @version: v1.0
 */
public interface IDepartmentService {
	/**
	 * 
	 * @Title: showDepartment @param @return @return List<Department> @throws
	 */
	public List<Department> showDepartment();

	/**
	 * 
	 * @Title: queryDepartmentsById @param @param
	 * depaNumber @param @return @return Department @throws
	 */
	public Department queryDepartmentsById(Integer depaNumber);

	/**
	 * 
	 * @Title: queryDepartmentsByName @param @param
	 * depaName @param @return @return List<Department> @throws
	 */

	public List<Department> queryDepartmentsByIdlist(Integer depaNumber);

	public List<Department> queryDepartmentsByName(String depaName);

	/**
	 * 
	 * @Title: updateByPrimaryKeySelective @param @param
	 * record @param @return @return String @throws
	 */
	public String updateByPrimaryKeySelective(Department record);

	/**
	 * 
	 * @Title: deleteByPrimaryKey @param @param
	 * depaNumber @param @return @return String @throws
	 */
	public String deleteByPrimaryKey(Integer depaNumber);

	/**
	 * 
	 * @Title: insertSelective @param @param record @param @return @return
	 * String @throws
	 */
	public String insertSelective(Department record);

	/**
	 * 获得数量
	 * 
	 * @return
	 */
	public int getCountDepartment();

	/**
	 * 分页
	 * 
	 * @param startPos
	 * @param pageSize
	 * @return
	 */
	public List<Department> findDepartmentPages(Integer toPageNo);

}
